...
const ToDoScreen = () => {
    ...
    const handleUpdateToDo = () => {
        if (editToDo) {
            updateTodo(editToDo.id, toDo, editToDo.date, editToDo.completed);
            setEditToDo(null);
    ...
    const handleToggleCompletion = (id, completed) => {
        ...
        if (currentItem) {
            updateTodo(id, currentItem.title, currentItem.date, !completed);
            loadToDoList();
        }
    };
    ...
    const renderToDo = ({ item, index }) => {
        ...
        return (
            <View style={styles.renderContainer}>
                <View style={{ flex: 1 }}>
                    <Text style={[styles.renderText, { textDecorationLine: item.completed ? 'line-through' : 'none', color: textColor }]}>{item.title}</Text>
                    <Text style={{ color: "black", fontSize: 13 }}>{item.date}</Text>
                </View>
                <IconButton
                    icon={item.completed ? 'checkbox-marked-circle-outline' : 'checkbox-blank-circle-outline'}
                    iconColor={item.completed ? '#1e90ff' : '#000'}
